#! /usr/bin/python
# -*- encoding: utf-8 -*-
## Implementation of flatNCE on angle prototypical loss (https://arxiv.org/pdf/2107.01152.pdf)
...
class LossFunction(nn.Module):
    ...
    def __init__(self, init_w=10.0, init_b=-5.0, **kwargs):
        ...
        print('Initialised FlattenAngleProto')
    ...
    def forward(self, x, label=None): #x.shape = torch.Size([20, 2, 12])
        ...
        for i in range(0, stepsize):
            temploss = torch.logsumexp(cos_sim_matrix[:][i] - cos_sim_matrix[i,i],0)
