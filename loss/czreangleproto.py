#! /usr/bin/python
# -*- encoding: utf-8 -*-
## Cross-Zero (w/ Positive Reciprocal) Angle Proto
...
class LossFunction(nn.Module):
    ...
    def __init__(self, init_w=10.0, init_b=-5.0, **kwargs):
        ...
        print('Initialised CrossZeroReciprocalAngleProto')
    ...
    def forward(self, x, label=None): #x.shape = torch.Size([20, 2, 12])
        ...
        for i in range(0, stepsize):
            temp += torch.logsumexp(torch.hstack([cos_sim_matrix[:,i][:i], -cos_sim_matrix[i,i], cos_sim_matrix[:,i][i+1:]]),0) - cos_sim_matrix[i,i]
        nloss = temp / stepsize
